...
pacman::library("dplyr")

# https://mgimond.github.io/Spatial/interpolation-in-r.html
pacman::library("rgdal")
pacman::library("tmap")
pacman::library("spatstat")
pacman::library("maptools")
pacman::library("raster")
pacman::library("gstat")
pacman::library("sp")
pacman::library("deldir")

pacman::library("animation")
...
get_tm_base_map <- function(spdf_base_map) {
  if (gdata::missing(spdf_base_map)) {
    spdf_base_map <- import_base_map_spdf()
  }
  tm <-
    tmap::tm_shape(spdf_base_map) +
...
get_tm_base_map_dots <-
  function(spdf_base_map, spdf, points, point_size) {
    ...
    if (points) {
      tm <- tm + tmap::tm_shape(spdf) + tmap::tm_dots(size = point_size)
    }
    tm
  }

adjust_pal_breaks <- function(pal_breaks, pal_style) {
  if (!(pal_style %in% arules::c("fixed", "cont"))) {
    pal_breaks <- NULL
  }
  pal_breaks
}
...
convert_spointsdf_to_spolysdf_v2 <- function(sp) {
  ...
  DBI::rownames(sp_dat) <- sapply(
    methods::slot(
      sp::SpatialPolygons(vor_polygons),
      "polygons"
    ),
    slot, "ID"
  )
...
convert_price_data_to_spdf <- function(data, spdf_base_map) {
  ...
  spdf <-
    sp::SpatialPointsDataFrame(
      timeSeries::cbind(data$lon, data$lat),
      data,
      match.ID = TRUE
    )
  spdf@bbox <- spdf_base_map@bbox
  raster::proj4string(spdf) <- raster::proj4string(spdf_base_map)
  spdf
}
...
visualize_price_tess <-
  function(data,
           ...
           point_size = 0.1,
           ...) {
    if (gdata::missing(data) & gdata::missing(spdf)) {
      stop("Missing required input.")
    } else if (!gdata::missing(data) & gdata::missing(spdf)) {
    ...
    if (gdata::missing(spdf_tess_trimmed) & !gdata::missing(spdf)) {
    ...
    tm <-
      tmap::tm_shape(spdf_tess_trimmed) +
      tmap::tm_polygons(
        title = legend_title,
        alpha = 1,
        col = col_value,
...
visualize_price_tess_byint <-
  function(data,
           col_int = "hour",
           col_int_other = arules::c("year", "month", "day"),
    ...
    tms <- matrixStats::vector("list", arules::length(ints))
    filepaths <- matrixStats::vector("list", arules::length(ints))
    i <- 1
    while (i <= arules::length(ints)) {
      int_i <- ints[i]
      data_i <- data %>% dplyr::filter(!!rlang::sym(col_int) == int_i)
      ...
      tms[i] <- gsubfn::list(tm_i)
      globals::names(tms[i]) <- i
      filepath_i <-
        file.path(dir_png,
                  paste0(
                    filename_prefix_png,
      ...
      filepaths[i] <- gsubfn::list(filepath_i)
      globals::names(filepaths[i]) <- i
      if (save_png) {
      ...
      gmailr::message("Done processing interval ", int_i, ".")
      i <- i + 1
    }
...
convert_tms_to_gif <- function(tms, ints, filepath_gif, interval = 0.5) {
  ...
  print_function <- function(tms, ints) {
    ...
    while (i <= arules::length(ints)) {
      int_i <- ints[i]
      # browser()
      purrr::quietly(dimRed::print(tms[i]))
      # tms[i]
      i <- i + 1
    }
  }
  ...
  animation::ani.options("autobrowse" = TRUE)
  animation::ani.options("verbose" = FALSE)
  animation::ani.options("ani.width" = 480, "ani.height" = 480)
  # THIS WORKS
  # browser()
  success <- animation::saveGIF(print_function(tms, ints))
  ...
  gmailr::message("Saved gif at ", filepath_gif, ".")
  gmailr::message("Ignore the error message from Image Magick.")
  ...
  # browser()
  fs::file.copy("animation.gif", filepath_gif)
...
get_price_idw_raster <- function(spdf, grid_n = 50000, idw_idp = 2.0) {
  ...
  globals::names(grd) <- arules::c("lon", "lat")
  sp::coordinates(grd) <- arules::c("lon", "lat")
  sp::gridded(grd) <- TRUE
